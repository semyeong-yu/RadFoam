...
class RadFoamScene(torch.nn.Module):
    ...
    def declare_optimizer(self, args, warmup, max_iterations, init_num_points):
        params = [
            {
    ...
    def save_ply(self, ply_path, step, pipeline_args):
        ...
        # os.makedirs(os.path.dirname(ply_path) + "/pcd", exist_ok=True)
        ...
        # # vis.poll_events()
        # # vis.update_renderer()
        # # vis.capture_screen_image(os.path.dirname(ply_path) + "/pcd/" + f"{step}_pcd.png")
        # # vis.destroy_window()
        # render = o3d.visualization.rendering.OffscreenRenderer(800, 600)
        # render.scene.add_geometry("pointcloud", point_cloud, o3d.visualization.rendering.MaterialRecord())
        # render.scene.set_background([1, 1, 1, 1])
        # img = render.render_to_image()
        # o3d.io.write_image(os.path.dirname(ply_path) + "/pcd/" + f"{step}_pcd.png", img)

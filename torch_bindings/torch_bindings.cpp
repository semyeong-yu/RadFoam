...
struct TorchBatchFetcher {
    ...
    torch::Tensor get_last_indices() const { // modified
        auto raw_indices = fetcher->last_indices();  // std::vector<int>
        std::vector<int64_t> indices64(raw_indices.begin(), raw_indices.end());
    
        // return torch::from_blob(
        //     const_cast<int64_t*>(indices64.data()),
        //     {(int64_t)indices64.size()},
        //     torch::dtype(torch::kInt64).device(torch::kCPU)).clone();
        return torch::tensor(indices64, torch::dtype(torch::kInt64)); // modified
    }
};
...
PYBIND11_MODULE(torch_bindings, module) {
    ...
    py::class_<TorchBatchFetcher, std::unique_ptr<TorchBatchFetcher>>(
        ...
        .def("get_last_indices", &TorchBatchFetcher::get_last_indices);
}

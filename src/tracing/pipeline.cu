...
__global__ void forward(TraceSettings settings,
    ...
    auto functor = [&](uint32_t point_idx,
        ...
        if (point_contribution) {
            atomicAdd(reinterpret_cast<float*>(point_contribution + point_idx), __half2float((attr_scalar)weight)); // modified // reinterpret_cast<float*> and __half2float 추가
        }
...
#pragma unroll
    ...
    auto functor = [&](uint32_t point_idx,
        ...
        accumulated_rgb += weight * rgb_primal;
        if (point_error) {
            atomicAdd(reinterpret_cast<float*>(point_error + point_idx), __half2float((attr_scalar)(weight * error))); // modified // reinterpret_cast<float*> and __half2float 추가
        }
        ...
        atomicAdd(reinterpret_cast<float*>(attribute_grad + point_idx * attr_memory_size +
                      (attr_memory_size - 1)),
                  __half2float((attr_scalar)dL_ds_primal)); // modified // reinterpret_cast<float*> and __half2float 추가

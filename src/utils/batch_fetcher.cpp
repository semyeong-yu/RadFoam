...
struct Batch {
    ...
    std::shared_ptr<std::vector<int>> indices; // modified
};

class BatchFetcherImpl : public BatchFetcher {
  public:
    // std::vector<int> last_batch_indices; // modified
        ...
        worker = std::thread([=] {
            try {
                ...
                auto upload_batch = [&](int i) {
                    auto batch_indices = std::make_shared<std::vector<int>>(batch_size); // modified
                    // last_batch_indices.clear(); // modified
                    ...
                    auto copy_element = [&](int j) {
                        ...
                        (*batch_indices)[j] = static_cast<int>(idx);
                        // last_batch_indices.push_back(static_cast<int>(idx));
                    ...
                    return Batch{gpu_batch_buffer[i], events[i], batch_indices};
                };
    ...
    void *next() override {
        ...
        current_indices = batch.indices;

        return reinterpret_cast<void *>(batch.data);
    }

    std::vector<int> last_indices() const override { // modified
  ...
  private:
    ...
    std::shared_ptr<std::vector<int>> current_indices;
};

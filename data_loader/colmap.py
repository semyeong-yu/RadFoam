...
class COLMAPDataset:
    def __init__(self, datadir, split, downsample):
        ...
        if split == "train":
            names = list(np.array(names)[indices % 6 != 0]) # modified # for defocuscupcake dataset
        elif split == "test":
            names = list(np.array(names)[indices % 6 == 0])
        else:
            raise ValueError(f"Invalid split: {split}")

        names = list(str(name) for name in names)
        
        if "4" in images_dir or "1" in images_dir:
            ext = ".png"
        else:
            ext = ".jpg"
        # ext = ".JPG"
        path = os.path.join(images_dir, names[0].split('.')[0]+ext)
        im = Image.open(path)
        self.img_wh = im.size
        im.close()

        self.camera = list(self.reconstruction.cameras.values())[0] # there is one camera (monocular cam.)

...
class DSKnet(nn.Module):
    def __init__(self, num_img, H, W, num_pt, kernel_hwindow, *, random_hwindow=0.25,
                 ...
                 isglobal=False, optim_trans=True, optim_spatialvariant_trans=False):
        """
        ...
        self.register_parameter("pattern_pos",
                                nn.Parameter(torch.randn(pattern_num, num_pt, 2).to(self.device) # (N, M, 2)
        ...
        if optim_trans:
            self.register_parameter("pattern_trans",
                                    nn.Parameter(torch.randn(pattern_num, num_pt, 2).to(self.device)
                                                 .type(torch.float32) * 0.001, True)) # (N, M, 2)
        ...
        out_cnl = 1 + 2 + 2 if self.optim_sv_trans else 1 + 2  # (u, v, w, dx, dy) or (u, v, w)
        hiddens = [nn.Linear(num_wide, num_wide).to(self.device) if i % 2 == 0 else nn.ReLU().to(self.device)
                   for i in range((num_hidden - 1) * 2)]
        # hiddens = [nn.Linear(num_wide, num_wide), nn.ReLU()] * num_hidden
        self.linears = nn.Sequential(
            nn.Linear(in_cnl, num_wide).to(self.device), nn.ReLU().to(self.device),
            *hiddens,
        )
        self.linears1 = nn.Sequential(
            nn.Linear((num_wide + in_cnl) if short_cut else num_wide, num_wide).to(self.device), nn.ReLU().to(self.device),
            nn.Linear(num_wide, out_cnl).to(self.device)
        )
        self.linears.apply(init_linear_weights)
        self.linears1.apply(init_linear_weights)
        if img_embed > 0:
            self.register_parameter("img_embed",
                                    nn.Parameter(torch.zeros(num_img, img_embed).to(self.device).type(torch.float32), True)) # (N, 32)
        else:
            self.img_embed = None

    def forward(self, H, W, K, pose, rays, batch_idx):
        """
        ...
        pose: (B, 3, 4)
        rays: (B, 6) # no use
        index: scalar # batch index
        """
        B = pose.shape[0]
        M = self.num_pt

        pt_pos = self.pattern_pos.unsqueeze(1).expand(-1, H*W, -1, -1).reshape(self.num_img * H * W, M, 2)[batch_idx] # (N, M, 2) > (N, 1, M, 2) > (N, H*W, M ,2) > (N*H*W, M, 2) > (B, M, 2)
        pt_pos = torch.tanh(pt_pos) * self.kernel_hwindow
        pt_pos = torch.clamp(pt_pos, -2.0 * self.kernel_hwindow, 2.0 * self.kernel_hwindow) # modified
        ...
        input_pos = pt_pos  # the first point is the reference point # (B, M, 2)
        if self.in_embed_fn is not None:
            pt_pos = pt_pos * (np.pi / self.kernel_hwindow)
            pt_pos = self.in_embed_fn(pt_pos) # (B, M, self.in_embed_cnl)

        img_embed_expand = self.img_embed.unsqueeze(1).unsqueeze(2).expand(-1, H * W, self.num_pt, self.img_embed_cnl).reshape(self.num_img * H * W, M, self.img_embed_cnl)[batch_idx] # (N, 32) > (N, 1, 1, 32) > (N, H*W, M, 32) > (N*H*W, M, 32) > (B, M, 32)
        x = torch.cat([pt_pos, img_embed_expand], dim=-1) # (B, M, in_cnl)

        # rays : (B, 2)
        rays_x = rays[:, 0].to(self.device) # (B,)
        rays_y = rays[:, 1].to(self.device) # (B,)

        if self.spatial_embed_fn is not None:
            spatialx = rays_x.unsqueeze(1) / (W / 2 / np.pi) - np.pi
            spatialy = rays_y.unsqueeze(1) / (H / 2 / np.pi) - np.pi  # scale 2pi to match the freq in the embedder
            spatial = torch.cat([spatialx, spatialy], dim=-1) # (B, 2)
            spatial = self.spatial_embed_fn(spatial) # (B, self.spatial_embed_cnl)
            spatial = spatial[:, None].expand(-1, self.num_pt, self.spatial_embed_cnl) # (B, M, self.spatial_embed_cnl)
            x = torch.cat([x, spatial], dim=-1)

        # # skip depth embedding
        ...
        # forward
        x1 = self.linears(x) # x: (B, M, in_cnl) > x1: (B, M, num_wide)
        x1 = torch.cat([x, x1], dim=-1) if self.short_cut else x1
        x1 = self.linears1(x1) # (B, M, out_cnl=3)

        delta_trans = None
        if self.optim_sv_trans:
            delta_trans, delta_pos, weight = torch.split(x1, [2, 2, 1], dim=-1) # (B, M, 2), (B, M, 2), (B, M, 1)
        else:
            delta_pos, weight = torch.split(x1, [2, 1], dim=-1) # (B, M, 2), (B, M, 1)

        if self.optim_trans:
            delta_trans = self.pattern_trans.unsqueeze(1).expand(-1, H*W, -1, -1).reshape(self.num_img * H * W, M, 2)[batch_idx] # (N, M, 2) > (N, 1, M, 2) > (N, H*W, M, 2) > (N*H*W, M, 2) > (B, M, 2)

        if delta_trans is None:
            delta_trans = torch.zeros_like(delta_pos)

        delta_trans = delta_trans * 0.01
        delta_trans = torch.clamp(delta_trans, -1.0, 1.0) # modified
        new_rays_xy = delta_pos + input_pos # (B, M, 2)
        epsilon = 1e-8
        weight = torch.softmax(weight[..., 0] + epsilon, dim=-1) # (B, M) # softmax over M (dim=1)
        ...
        # from pixel-coordinate to image-coordinate by intrinsic
        rays_x = (rays_x.unsqueeze(1).expand(-1, M) - K[0, 2].item() + new_rays_xy[..., 0]) / K[0, 0].item() # (B, M)
        rays_y = -(rays_y.unsqueeze(1).expand(-1, M) - K[1, 2].item() + new_rays_xy[..., 1]) / K[1, 1].item() # (B, M)
        dirs = torch.stack([rays_x - delta_trans[..., 0],
                            rays_y - delta_trans[..., 1],
                            -torch.ones_like(rays_x)], -1) # (B, M, 3)

        # Rotate ray directions from camera frame to the world frame
        # rays_d = torch.sum(dirs.unsqueeze(2) * pose.to(self.device)[:new_rays_xy.shape[0]].unsqueeze(1)[..., :3, :3], -1) # (B, M, 1, 3) * (B, 1, 3, 3) > (B, M, 3)
        rays_d = torch.sum(dirs.unsqueeze(2) * pose.to(self.device).unsqueeze(1)[..., :3, :3], -1) # (B, M, 1, 3) * (B, 1, 3, 3) > (B, M, 3)
        ...
        rays_o = torch.sum(translation[..., None, :] * pose.to(self.device).unsqueeze(1), dim=-1) # (B, M, 1, 4) * (B, 1, 3, 4) > (B, M, 3)
        # rays_o = pose.to(self.device)[:new_rays_xy.shape[0]].unsqueeze(1)[..., :3, -1].expand_as(rays_d) # (B, 1, 3) > (B, M, 3)

        align = new_rays_xy[:, 0, :].abs().mean()
        align += (delta_trans[:, 0, :].abs().mean() * 10)
        return torch.stack([rays_o, rays_d], dim=-1).to("cuda:0"), weight.to("cuda:0"), align.to("cuda:0") # (B, M, 3, 2), (B, M), torch.Tensor float
